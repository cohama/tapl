...
#[derive(Debug, Clone)]
pub struct NameBind;
impl Binding for NameBind {}

#[derive(Debug, Clone)]
pub struct VarBind(Type);
impl Binding for VarBind {}

#[derive(Debug, Clone)]
pub struct Context<T: Binding>(Vec<(String, T)>);

impl Context<NameBind> {

    pub fn new<U: AsRef<str>>(names: &[U]) -> Context<NameBind> {
        Context(names.iter().map(|name| (name.as_ref().to_string(), NameBind)).collect())
    }

    fn pick_fresh_name(&self, name: &str) -> (Context<NameBind>, String) {
...
impl Context<VarBind> {
    pub fn new<S: AsRef<str>>(binds: &[(S, VarBind)]) -> Context<VarBind> {
        Context(binds.iter().map(|&(ref name, ref t)| (name.as_ref().to_string(), t.clone())).collect())
    }

    fn add_binding<S: Into<String>>(&self, name: S, bind: VarBind) -> Context<VarBind> {
        let mut v = self.clone();
        v.0.push((name.into(), bind));
        v
    }

    fn get_type(&self, n: usize) -> &Type {
        &(self.0[self.len() - 1 - n].1).0
    }
}
...
impl Term {
    pub fn show(&self, ctx: &Context<NameBind>) -> String {
        use self::Term::*;
        match *self {
            Var(x, n) => {
                if ctx.len() == n {
